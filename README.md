...
本工具主要用于室内 RGB-D 数据标注场景。用户可以在 2D 图像上通过点选、框选等方式调用 SAM 生成候选掩膜，并从多个候选 mask 中选择效果最好的分割结果。随后系统会对掩膜进行后处理，并基于相机内参、外参或投影矩阵，将 2D 掩膜区域对应到 3D 点云中，实现 2D 与 3D 标注结果的同步生成。
