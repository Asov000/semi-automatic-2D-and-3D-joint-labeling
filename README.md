...
点击 UI 中的“交互式编辑 3D 框”后，会同时打开 Open3D 编辑窗口和 Qt 控制台。控制台左侧显示快捷键说明，中间显示框列表和当前框详情，右侧提供面缩放、旋转、平移、删除和保存按钮。
...
N / P      切换下一个 / 上一个框
1..6       选择面：+X, -X, +Y, -Y, +Z, -Z
E / W      按右侧步长扩展 / 缩进当前选中的面
D / A      按右侧角度正向 / 反向旋转当前框
方向键     在世界 X/Y 平面移动当前框
PageUp/Down 沿世界 Z 轴移动当前框
X/Delete   删除当前框
```

保存不再绑定 `Q` 或 `S`，需要点击右侧“保存并关闭”。如果直接关闭控制台或 Open3D 视窗，则本次修改不会写入缓存。
